...
export const TodoRow = forwardRef<HTMLDivElement, Props>(
  ({ tableId, rowId, selectedTodo }, ref) => {
    ...
    const toggleTodo = UIReact.useSetCellCallback(
      tableId,
      rowId,
      "completed",
      () => !todo.completed,
    ...
    const updateTodoText = UIReact.useSetCellCallback(
      tableId,
      rowId,
      "text",
      (text: string) => text,
        ...
        <ContextMenu>
          <ContextMenuTrigger asChild>
            ...
            >
              <div
                className={cn(
                  "absolute inset-0 z-[-1] rounded-lg focus:bg-zinc-600/50 group-hover:bg-zinc-600/50",
                  selectedTodo === todo.id && "bg-zinc-600/50"
                )}
              />
              <div
                key={todo.id}
